...
export function SuperAdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    setError("");
    try {
      const [businessesData, statsData, auditsPage] = await Promise.all([
        apiService.getBusinessesAdmin().catch(() => []),
        // Stats endpoint
        (async () => {
          try {
            const res = await apiService.get<any>?.("/api/admin/stats");
            return res as PlatformStats;
          } catch {
            return null;
          }
        })(),
        // Recent audit logs (paged)
        (async () => {
          try {
            const res = await apiService.get<any>?.("/api/audit/search", {
              params: { size: 20, page: 0 },
            });
            return res as { content: ActivityItem[] };
          } catch {
            return { content: [] };
          }
        })(),
      ...
      setStats(statsData);
      setActivities(
        Array.isArray(auditsPage?.content) ? auditsPage.content : []
      );
    } catch (err: any) {
  ...
  const handleCreateOwner = async (e: React.FormEvent) => {
    ...
    if (ownerPassword !== ownerPasswordConfirm) {
      setOwnerFormError("Passwords do not match");
      return;
    }

    if (ownerPassword.length < 6) {
      setOwnerFormError("Password must be at least 6 characters");
      return;
    }
    ...
    try {
      ...
      setSuccess(
        `Owner "${ownerName}" created successfully! ${
          businessName
            ? `Business "${businessName}" also created.`
            : "Owner can create their business after login."
        }`
      );
      setOwnerName("");
      setOwnerEmail("");
      ...
      setOwnerFormError(err?.message || "Failed to create owner");
    }
  };
  ...
  const handleToggleBusiness = async (
    ...
    try {
      await apiService.setBusinessStatus(businessId, !currentStatus);
      setSuccess(
        `Business ${!currentStatus ? "activated" : "deactivated"} successfully!`
      );
      await fetchData();
  ...
  const filteredBusinesses = Array.isArray(businesses)
    ? businesses.filter((business) => {
        ...
        const ownerName = (business.owner?.fullName || "").toLowerCase();
    ...
    <div className="p-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-2">
          Monitor all businesses and platform metrics
        </p>
      </div>
          ...
          <div className="text-center py-12 text-gray-500">
            ...
            <p>
              {searchTerm
                ...
                : "No businesses yet. Create your first business owner to get started."}
            </p>
          </div>
        ) : (
                  ...
                  >
                    ...
                    <td className="py-3 px-4">
                      <div className="flex items-center gap-2">
                        <button
                          onClick={() =>
                            handleToggleBusiness(business.id, business.active)
                          }
          ...
          <div className="bg-white rounded-xl shadow-xl p-8 max-w-md w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <form onSubmit={handleCreateOwner} className="space-y-4">
              ...
              <div>
                ...
                />
                <p className="text-gray-500 text-sm mt-1">
                  If provided, a business will be created and assigned to this
                  owner
                </p>
              </div>
